...
public class BunkerResourceManager : MonoBehaviour
{
    ...
    public BunkerResourceSaveData GetSaveData()
    {
        return new BunkerResourceSaveData
        {
            scrap = _scrap,
    ...
    public void LoadFromData(BunkerResourceSaveData data)
    {
        if (data == null)
            return;

        _scrap = Mathf.Max(0, data.scrap);
        _electricity = Mathf.Max(0, data.electricity);
        _water = Mathf.Max(0, data.water);
        _food = Mathf.Max(0, data.food);
    }
...
[Serializable]
public class BunkerResourceSaveData
{
    public int scrap;
